...
void main_window::select_directory()
{
    ...
    connect(counter, SIGNAL(finish1(QVector<my_file>)),
            this,
            SLOT(onFinish1(QVector<my_file>)));
...
void main_window::onFinish1(QVector<my_file> q)
{
    count = q.size();
    files = q;
    for (int i = 0; i < files.size(); i++)
        scanner.addPath(files[i].filename);
    ui->label_2->setText("Finished in " + QString::number(t.elapsed()) + " ms. Found " + QString::number(q.size()) + "  ");
    ui->label_2->setHidden(false);
    ui->label->setHidden(false);
    ui->lineEdit->setHidden(false);
    ui->actionIndex_Directory->setDisabled(false);
